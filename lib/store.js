...
const { getShape, sanitizeCustomShape, suggestShape } = require('./tactile');
...
class Store {
  ...
  normalizeItem(input, id) {
    // 外形来自模型生成时，真正的几何在 tag_shape_custom 里；tag_shape 退化为
    // 「最接近的内置外形」，作为几何不可用时的兜底与粗略的类别线索。
    const customShape = sanitizeCustomShape(input.tag_shape_custom);
    const item = {
      id,
