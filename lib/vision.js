...
5. 如果照片里的物品涉及吃药、用药剂量、燃气或需要专业判断，suggested_actions 给空数组 []，
   在 safety_notes 里写明「具体步骤必须由医生、药师或专业人员确认后填写」。
6. 不要做任何安全保证。安全注意事项不确定的一律写「待确认」。
7. **照片里看不到控制面板时必须诚实认输**：输出
...
8. name_guess、category_guess、summary、placement_note 必须给。
...
9. category_guess 必须是给定清单里的原词，原样输出，不要自创类别。
10. tag_hint 要用「从标签向右约两指宽」这种以标签为原点、手指能数出距离的说法，
    因为用户唯一能先摸到的就是标签本身。
11. 如果照片里拍到的其实是屏幕、纸面、说明书或另一台手机的界面（也就是「物件的照片」
...
async function candidates({ image, name, category, notes, intent } = {}) {
  ...
  if (hasImage && vlmEnabled()) {
    try {
      ...
      const notice = generatedSteps
        ...
        ? '照片中未识别到控制件或控制面板，因此未生成控制件清单与操作步骤。可对准控制面板重拍，或在下方手动添加。'
        : '控制件已列出，但该类物品涉及专业判断（用药、剂量、燃气等），操作步骤需由录入人或专业人员填写。';
      // 模型给的自动填写表 + 用户已填值兜底
      ...
      return {
        ...
        safety_notes: modelNotes.length
          ? risk
            ? ['高风险类别：档案未记录的操作，助手不予建议。', ...modelNotes].slice(0, 6)
            : modelNotes
          : base.safety_notes,
      ...
      return {
        ...
        notice: `未能识别照片（${e.message}）。以下内容为通用模板，请全部核对，或稍后重试。`,
        vlm_error: e.message,
      };
    }
  }
  ...
  return {
    ...
    notice: hasImage
      ? '照片将正常存入档案。当前服务器未启用图像识别能力，以下内容依据「类别」生成，非照片识别结果，请逐条核对。'
      : '以下内容依据名称与类别生成，请逐条核对后再保存。',
  };
}
...
async function look(item, image) {
  ...
  try {
    ...
    return {
      ok: false,
      source: 'vlm-error',
      answer: `未能识别照片（${e.message}）。可重新拍摄，或改用「语音对话」提问。`,
      details: [],
      matches_item: null,
      confidence: null,
    };
